...
export async function createGoogleMeetEvent({ resume, job, interview }) {
  ...
  const attendees = resume.email ? [{ email: resume.email, displayName: resume.name }] : [];
  ...
  return {
    ...
    meetLink: result.hangoutLink || result.conferenceData?.entryPoints?.find((item) => item.uri)?.uri || "",
  };
}
